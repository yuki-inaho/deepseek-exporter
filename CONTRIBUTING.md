...
Please **do not include changes to `dist/` in your PR**. The published userscript (`dist/deepseek.user.js`) is built and committed automatically by CI, which keeps the build reproducible from source and guarantees the distributed script has not been tampered with.
